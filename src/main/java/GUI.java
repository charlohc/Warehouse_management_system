...
public class GUI {
    ...
    private void discountItem() {
        ...
        if (itemRegister.discount(itemNumberDiscount.trim(), Integer.parseInt(discountInPercentage)) == -1) {
            ...
            System.out.println("The price before discount: " + itemCalculatedDiscount.getPrice());
            System.out.println("The price after discount: " + (itemCalculatedDiscount.getPrice() - discountInKr));
        }
    }

    private void removeItemTypeFromInventory() {
        System.out.println("Here you can remove an item type and all of the corresponding items from the warehouse.");
        System.out.println("You will need to type in the item number of the item,\n"
        ...
        if (itemRegister.removeItemFromInventory(itemNumberRemoveItem.trim())) {
            ...
            System.out.println(ANSI_RED + "The removal was unsuccessful! \n"
                    + "There is no item with the item number: " + itemNumberRemoveItem + ANSI_RESET);
        }
    }
    ...
    private void findItemBasedOnDescription() {
        ...
        while (searchingDescription) {
            ...
            if (itemRegister.findItemBasedOnDescription(descriptionSearchInput).size() == 0) {
                System.out.println(ANSI_RED + "Could not find any item matching your description.." + ANSI_RESET);
                System.out.println("-".repeat(60));
                System.out.println("Do you want another search?"
                        + "\ntype 1 to return to main menu, press another key to keep searching");
                ...
                System.out.println(ANSI_GREEN + "These are the item(s) matching your description!" + ANSI_RESET);
                System.out.println(itemRegister.findItemBasedOnDescription(descriptionSearchInput).toString());
                searchingDescription = false;
            }
        }
    }
    ...
    private void addNewItem(){
        ...
        while (priceBlankOrNegative) {
            ...
            if(!priceInput.matches("[0-9]+") || item.lengthOfNumberInputInteger(Integer.parseInt(priceInput)) == 0 || priceInput.trim().equals("0")) {
                System.out.println( ANSI_RED + "Price cannot be blank or contain special characters or be set to zero! "
                        + "Try again... \n" + ANSI_RESET);
            } else {
        ...
        while (numbersInWarehouseBlankOrNegative) {
            ...
            if(!numbersInWarehouseInput.matches("[0-9]+")|| item.lengthOfNumberInputInteger(Integer.valueOf(numbersInWarehouseInput)) == 0
                    || numbersInWarehouseInput.trim().equals("0")) {
                System.out.println(ANSI_RED + "The numbers of items in warehouse cannot be blank or set as negative or zero, or contain special characters!"
                        + " Try again... \n" + ANSI_RESET);
            } else {
